...
class TableViewController: UITableViewController {
	...
	@objc func filterPetitions() {
		...
		ac.addTextField()
		ac.addAction(UIAlertAction(title: "Add Filter", style: .default) { [weak self, weak ac] _ in
			guard let theFilter = ac?.textFields?[0].text else { return }
			self?.performSelector(inBackground: #selector(self?.filterList), with: theFilter)
//			self?.filterListDispatch(theFilter)
		})
...
//			urlString = "https://www.hackingwithswift.com/samples/petitions-2.json"
	...
	func filterListDispatch(_ theFilter: String) {
		performSelector(inBackground: #selector(filterList), with: theFilter)

	}

	@objc func filterList(_ theFilter: String) {
		...
		filteredPetitions = newFilteredPetitions
		tableView.performSelector(onMainThread: #selector(UITableView.reloadData), with: nil, waitUntilDone: false)
	}
}
